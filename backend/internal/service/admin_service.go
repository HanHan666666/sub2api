...
type CreateGroupInput struct {
	...
	IsExclusive      bool
	SubscriptionType string   // standard/subscription/per_request
...
func (s *adminServiceImpl) CreateGroup(ctx context.Context, input *CreateGroupInput) (*Group, error) {
	...
	// per_request 模式验证：必须有单价或次数限额
	if subscriptionType == SubscriptionTypePerRequest {
		hasPrice := input.PerRequestPrice != nil && *input.PerRequestPrice > 0
		hasLimit := (input.DailyLimitRequests != nil && *input.DailyLimitRequests > 0) ||
			(input.WeeklyLimitRequests != nil && *input.WeeklyLimitRequests > 0) ||
			(input.MonthlyLimitRequests != nil && *input.MonthlyLimitRequests > 0)
		if !hasPrice && !hasLimit {
			return nil, fmt.Errorf("per_request mode requires either per_request_price or at least one request limit")
		}
	}
	...
	group := &Group{
		...
		DailyLimitRequests:              input.DailyLimitRequests,
		WeeklyLimitRequests:             input.WeeklyLimitRequests,
		MonthlyLimitRequests:            input.MonthlyLimitRequests,
...
func (s *adminServiceImpl) UpdateGroup(ctx context.Context, id int64, input *UpdateGroupInput) (*Group, error) {
	...
	if input.SubscriptionType != "" {
		// 禁止修改已存在分组的 subscription_type
		if input.SubscriptionType != group.SubscriptionType {
			return nil, fmt.Errorf("subscription_type cannot be changed after group creation")
		}
		group.SubscriptionType = input.SubscriptionType
	}
	// per_request 模式验证：必须有单价或次数限额
	if group.SubscriptionType == SubscriptionTypePerRequest {
		hasPrice := input.PerRequestPrice != nil && *input.PerRequestPrice > 0
		hasLimit := (input.DailyLimitRequests != nil && *input.DailyLimitRequests > 0) ||
			(input.WeeklyLimitRequests != nil && *input.WeeklyLimitRequests > 0) ||
			(input.MonthlyLimitRequests != nil && *input.MonthlyLimitRequests > 0)
	...
	// 按次计费配置
	group.PerRequestPrice = input.PerRequestPrice
	group.DailyLimitRequests = input.DailyLimitRequests
	group.WeeklyLimitRequests = input.WeeklyLimitRequests
	group.MonthlyLimitRequests = input.MonthlyLimitRequests
